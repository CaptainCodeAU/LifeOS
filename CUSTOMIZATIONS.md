...
|---|---|---|---|
| `LIFEOS/install/LIFEOS/PULSE/VoiceServer/voice.ts` | Added an `else if` branch in `sendNotification()`: when no ElevenLabs key is configured, calls the new local engine (`gk-localSay.ts`) instead of going silent. Also raised `RATE_LIMIT` 10→300 (never silently drop a notification), and gave `playAudio()` a `format` parameter (default `"mp3"`, unchanged for the ElevenLabs caller) so the temp file's extension matches the actual bytes — found via a real `/notify` call: `afplay` failed ("Problem scanning for packets") trying to parse real AIFF-C content as MP3, because the temp filename was hardcoded `.mp3` and every prior caller had always genuinely been MP3. | `sendNotification` is the one place that knows whether the paid path succeeded — the branch has to live here. `playAudio` is shared playback plumbing both engines need — no alternate location. Kept to the smallest possible diff each time. | If upstream ever ships its own local-engine hook point, the branch could shrink to a single call into it. |
